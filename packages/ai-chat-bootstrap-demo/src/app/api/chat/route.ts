...
export async function POST(req: Request) {
  try {
    ...
    const deserializedTools = tools?.reduce((acc: Record<string, ReturnType<typeof tool>>, serializedTool: SerializedTool) => {
      acc[serializedTool.name] = tool({
        description: serializedTool.description,
        inputSchema: jsonSchema(serializedTool.inputSchema as Record<string, unknown>),
    ...
    if (Object.keys(deserializedTools).length > 0) {
      ...
      Object.entries(deserializedTools).forEach(([name, toolDef]) => {
        systemMessageParts.push(`- ${name}: ${(toolDef as { description: string }).description}`)
      })
    }
