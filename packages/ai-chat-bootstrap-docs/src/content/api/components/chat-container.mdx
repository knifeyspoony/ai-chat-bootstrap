...
> All AI configuration (API endpoint, system prompt, models, MCP, threading, compression, etc.) is supplied via the `UseAIChatOptions` keys: `transport`, `messages`, `threads`, `features`, `mcp`, `models`, and `compression`.
