...
> All AI configuration (API endpoint, system prompt, models, MCP, threading, etc.) is supplied via the `UseAIChatOptions` keys: `transport`, `messages`, `thread`, `features`, `mcp`, and `models`.
