...
| `models` | `ChatModelOption[]` | No | Available models for selection |
| `selectedModelId` | `string` | No | Initially selected model ID |
| `chainOfThoughtEnabled` | `boolean` | No | Enable reasoning display mode |
| `mcpEnabled` | `boolean` | No | Enable MCP server tools |
...
| `models` | `ChatModelOption[]` | Available models for selection |
| `model` | `string \| undefined` | Currently selected model ID |
| `chainOfThoughtEnabled` | `boolean` | Whether reasoning display is enabled |
| `mcpEnabled` | `boolean` | Whether MCP server tools are enabled |
...
const chat = useAIChat({
  ...
  models: [
    ...
    { id: "gpt-3.5-turbo", name: "GPT-3.5", description: "Faster, less expensive" },
...
// Model selection dropdown appears automatically in ChatContainer
```
...
// Reasoning blocks are displayed automatically when present
```
...
function ChatWithAutoSubmit() {
  const chat = useAIChat({ api: "/api/chat" });

  // Auto-submit when user presses Enter (without Shift)
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Enter' && !e.shiftKey && chat.input.trim()) {
        e.preventDefault();
        chat.handleSubmit();
      }
    };
