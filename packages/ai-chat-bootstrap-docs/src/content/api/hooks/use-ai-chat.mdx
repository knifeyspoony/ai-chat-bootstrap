...
| `thread.id` | `string` | auto | Explicit thread id to load or create |
| `thread.scopeKey` | `string` | `undefined` | Scope identifier used when listing threads |
| `thread.autoCreate` | `boolean` | `true` | Create a thread automatically when none exists |
| `thread.warnOnMissing` | `boolean` | `false` | Log warnings when a referenced thread is missing |
| `thread.title.api` | `string` | `""` | Endpoint used to upgrade thread titles |
| `thread.title.sampleCount` | `number` | `8` | Number of messages sent to the title endpoint |
...
const chat = useAIChat({
  thread: {
    scopeKey: projectId,
    autoCreate: true,
    title: { api: "/api/threads/title", sampleCount: 6 },
  },
});
