...
Main chat interface component with built-in `useAIChat` state. It accepts all `UseAIChatOptions` (via `transport`, `messages`, `thread`, `features`, `models`, `mcp`) plus UI configuration groups such as `header`, `ui`, `suggestions`, `commands`, `threads`, and `assistantActions`. Always prefer the structured configuration groups when wiring new code.
