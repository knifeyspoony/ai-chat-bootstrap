...
The `useAIFocus` hook provides everything needed to manage focus items:
...
Focus items are **automatically included** in chat requests when using `useAIChat`. Access them in your API route:
...
The `useAIFocus` hook provides actions and reactive state for managing focus items:

<LiveCodeExample code={FOCUS_ITEMS_HOOK_SOURCE}>
  <div className="p-4 bg-gray-50 dark:bg-gray-900 rounded-lg">
    <div className="text-sm text-gray-600 dark:text-gray-400">
      Complete API reference for the useAIFocus hook
    </div>
  </div>
</LiveCodeExample>
...
| Method | Description |
|--------|-------------|
| `setFocus(id, item)` | Add an item to focus. Item should include `id`, `label`, optional `description`, and optional `data` |
| `clearFocus(id)` | Remove a specific item from focus by ID |
| `clearAllFocus()` | Remove all items from focus |
| `getFocus(id)` | Retrieve a specific focused item by ID |
