...
const { transformGlobalsCss } = require("./transform-globals-css");
...
async function scaffold({ projectName, tailwindNative, localPath }) {
  ...
  try {
    ...
    if (installingIntoExistingProject) {
      log(
        `Installing ai-chat-bootstrap into existing project at ${projectName} ...`
      );
    ...
    await writeFileWithConfirmation(suggestionsRoutePath, suggestionsTemplate);
    ...
    await writeFileWithConfirmation(compressionRoutePath, compressionTemplate);
    ...
    await writeFileWithConfirmation(threadTitleRoutePath, threadTitleTemplate);
    ...
    if (installingIntoExistingProject) {
      ...
      try {
        ...
        if (fs.existsSync(layoutPath)) {
          ...
          const layoutUpdated = await writeFileWithConfirmation(
            layoutPath,
            layout
          );
          if (layoutUpdated) {
    ...
    try {
      ...
      const transformed = transformGlobalsCss(existing, { tailwindNative });
      if (transformed !== existing) {
        ...
        if (globalsUpdated) {
          log("Updated globals.css imports and tokens");
        }
      }
    } catch {
