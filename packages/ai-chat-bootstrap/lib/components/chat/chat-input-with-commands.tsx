...
import { useAIChatCommandsStore, type ChatCommand, type UIChatCommand, type AIChatCommand } from "../../stores/commands"
import { hasRequiredParameters, generatePlaceholder, parseArgsToParams, getCurrentParameterIndex, hasAllRequiredParams } from "../../utils/command-utils"
...
export const ChatInputWithCommands = ({
  ...
  const [parameterValidationError, setParameterValidationError] = useState<string | null>(null)
  const { getMatchingCommands, executeCommand, getCommand } = useAIChatCommandsStore()
    ...
    if (isCommandInput) {
      ...
      if (args.length > 0 || cmdText.endsWith(' ')) {
        ...
        if (command && hasRequiredParameters(command.parameters)) {
          ...
          const textarea = document.querySelector('textarea[data-testid="chat-input"], textarea') as HTMLTextAreaElement
          const cursorPos = textarea?.selectionStart || value.length
          const paramIndex = getCurrentParameterIndex(value, cursorPos, command.parameters)
    ...
    if (showCommands && matchingCommands.length > 0 && value.startsWith('/')) {
      ...
      if (e.key === 'ArrowDown') {
        ...
        setSelectedCommandIndex(prev => 
          (prev + 1) % matchingCommands.length // Wrap around to beginning
        )
        return
      }
  ...
  const matchingCommands = enableCommands && showCommands 
    ? getMatchingCommands(commandQuery)
    : []

  // Reset selected index if it's out of bounds when commands change
  useEffect(() => {
    if (selectedCommandIndex >= matchingCommands.length && matchingCommands.length > 0) {
      setSelectedCommandIndex(0)
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (showCommands && matchingCommands.length > 0 && value.startsWith('/')) {
      e.preventDefault()
      handleCommandSelect(matchingCommands[selectedCommandIndex] || matchingCommands[0])
      return
    }
    ...
    if (value.startsWith('/')) {
      ...
      if (command) {
        ...
        if (!hasAllRequiredParams(argsString, command.parameters)) {
          setParameterValidationError('Please fill in all required parameters before executing the command')
          return
        }
